...
# Dependency graph khớp với k8s_real_env_5n8s.py
# Kịch bản chính: 5 nodes / 8 services
DEPENDENCIES = [
    ("frontend", "auth", 0.90),
    ("frontend", "catalog", 0.85),
    ("frontend", "cart", 0.80),
    ("catalog", "recommendation", 0.65),
    ("catalog", "database", 0.70),
    ("cart", "payment", 0.90),
    ("payment", "database", 0.95),
    ("payment", "shipping", 0.75),
    ("recommendation", "database", 0.50),
]
...
def main():
    ...
    parser.add_argument("--output", default="results/real_inter_service_latency_5n8s.csv")
    ...
    for name in required:
        p = pods[name]
        print(f"{name:<16} ip={p['ip']:<15} node={p['node']}")
    print("=" * 80)
